...
class RecurrentRGCN(nn.Module):
    def __init__(self, decoder_name, encoder_name, num_ents, num_rels, num_static_rels, num_words, num_times, time_interval, h_dim, opn, history_rate, sequence_len, num_bases=-1, num_basis=-1,
        ...
        self.entity_decoder = nn.Sequential(
            nn.Linear(4096, 1024),  # 输入层到隐藏层
            nn.ReLU(),  # 隐藏层激活函数
            nn.Linear(1024, h_dim)  # 隐藏层到输出层
        )
        ...
        self.decoder_gate_weight = nn.Parameter(torch.Tensor(2*h_dim, 1))    
        ...
        self.decoder_gate_weight_nhis = nn.Parameter(torch.Tensor(2*h_dim, 1))    
        ...
        self.decoder_gate_weight_all = nn.Parameter(torch.Tensor(2*h_dim, 1))    
        ...
        if decoder_name == "timeconvtranse":
            ...
            self.decoder_ob1_llm = TimeConvTransE(num_ents, h_dim, input_dropout, hidden_dropout, feat_dropout)
            self.decoder_ob2_llm = TimeConvTransE(num_ents, h_dim, input_dropout, hidden_dropout, feat_dropout)
    ...
    def predict(self, test_graph, num_rels, static_graph, test_triplets, entity_history_vocabulary, rel_history_vocabulary, use_cuda):
        ...
        with torch.no_grad():
            ...
            embedding_llm=F.normalize(llm_entity_emb)
            r_emb_llm=F.normalize(llm_relation_emb)
            ...
            #score_rel_r_llm = self.rel_raw_mode(embedding_llm, r_emb, time_embs, all_triples)
            #score_rel_h_llm = self.rel_history_mode(embedding_llm, r_emb, time_embs, all_triples, rel_history_vocabulary)
            score_r_llm = self.raw_mode_llm(embedding_llm, r_emb_llm, time_embs, all_triples)
            score_h_llm = self.history_mode_llm(embedding_llm, r_emb_llm, time_embs, all_triples, entity_history_vocabulary)
            ...
            score_llm=self.history_rate * score_h_llm + (1 - self.history_rate) * score_r_llm
