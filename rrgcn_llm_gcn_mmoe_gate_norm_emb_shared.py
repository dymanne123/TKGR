...
class RecurrentRGCN(nn.Module):
    ...
    def forward1(self, g_list, static_graph, use_cuda):
        ...
        if self.use_static:
            static_graph = static_graph.to(self.gpu)
            static_graph.ndata['h'] = torch.cat((self.dynamic_emb1, self.words_emb1), dim=0)  # 演化得到的表示，和wordemb满足静态图约束
        ...
        for i, g in enumerate(g_list):
            ...
            if i == 0:
                ...
                self.h_0 = self.relation_cell_11(x_input, self.emb_rel)    # 第1层输入
                self.h_0 = F.normalize(self.h_0) if self.layer_norm else self.h_0
            else:
                x_input = torch.cat((self.emb_rel1, x_input), dim=1)
                self.h_0 = self.relation_cell_11(x_input, self.h_0)  # 第2层输出==下一时刻第一层输入
                self.h_0 = F.normalize(self.h_0) if self.layer_norm else self.h_0
    ...
    def make_embedding(self,h_dim):
        ...
        # 读取 entity2id.txt 文件，获取 word_id 对应的 word
        word_id_to_word = {}
        ...
        #print(111)
        # 使用语言模型编码每个 word 并将其用于初始化 words_emb
        with torch.no_grad():
            for word_id, word in word_id_to_word.items():
                ...
                word_embedding = outputs.last_hidden_state.mean(dim=1)  # 取所有 token 的平均值作为整个句子的表示
    ...
    def predict(self, test_graph,num_rels, static_graph, test_triplets,mask, t, use_cuda,print_score=False):
        with torch.no_grad():
            ...
            rel_embedded = r_emb1[all_triples[:, 1]]
            ...
            e1_embedded_all = F.tanh(embedding1_nhis)
            e1_embedded_nhis = e1_embedded_all[all_triples[:, 0]]
            rel_embedded_nhis = r_emb1_nhis[all_triples[:, 1]]
    ...
    def get_loss(self, glist, triples,mask, static_graph, t,use_cuda):
        ...
        e1_embedded_all = F.tanh(pre_emb1_nhis)
        e1_embedded = e1_embedded_all[all_triples[:, 0]]
        rel_embedded = r_emb1_nhis[all_triples[:, 1]]
        ...
        if self.entity_prediction:
           ...
           #print(scores_ob_his.shape)
            ...
            candidate_emb_nhis=F.tanh(pre_emb1)
